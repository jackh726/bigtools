...
impl BigBedWrite {
    ...
    async fn process_group<I>(
        ...
        while let Some(current_val) = group.next()? {
            ...
            if current_val.start > current_val.end {
                return Err(WriteGroupsError::InvalidInput(format!("Invalid bed: {} > {}", current_val.start, current_val.end)));
            }
            if current_val.start >= chrom_length {
                return Err(WriteGroupsError::InvalidInput(format!("Invalid bed: `{}` is greater than the chromosome ({}) length ({})", current_val.start, chrom, chrom_length)));
            }
            ...
            let add_interval_to_summary = move |overlap: &mut VecDeque<Value>, summary: &mut Option<Summary>, item_start: u32, item_end: u32, next_start_opt: Option<u32>| {
                ...
                debug_assert!(overlap.front().map(|f| f.start == item_start).unwrap_or(true));
            ...
            add_interval_to_summary(&mut state_val.overlap, &mut summary, current_val.start, current_val.end, group.peek().map(|v| v.start));
            ...
            for (i, zoom_item) in state_val.zoom_items.iter_mut().enumerate() {
                ...
                while overlap.front().map(|f| f.start < next_start).unwrap_or(false) {
                    ...
                    loop {
                        ...
                        let (zoom2, _) = zoom_item.live_info.get_or_insert((ZoomRecord {
                            chrom: chrom_id,
                            start: add_start,
                            end: add_start,
                            summary: Summary {
                        ...
                        if add_end == next_end {
                            zoom_item.records.push(zoom_item.live_info.take().map(|(mut zoom_item, total_items)| { zoom_item.summary.total_items = total_items; zoom_item }).unwrap());
                        }
    ...
    pub fn begin_processing_chrom<I: 'static>(
        ...
        mut pool: ThreadPool,
        options: BBIWriteOptions,
    ) -> io::Result<ChromGroupRead> where I: ChromValues<BedEntry> + Send {
        let (
            ChromProcessingInput {
                zooms_channels,
                ftx,
        ...
        let (f_remote, f_handle) = BigBedWrite::process_group(zooms_channels, ftx, chrom_id, options, pool.clone(), group, chrom, chrom_length).remote_handle();
