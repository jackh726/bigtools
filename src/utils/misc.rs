...
pub fn bigwig_average_over_bed<R: Reopen<S> + 'static, S: SeekableRead + 'static>(
    ...
    let iter = std::iter::from_fn(move || -> Option<io::Result<BigWigAverageOverBedEntry>> {
        ...
        return match (|| {
            ...
            let cols = line.trim_end().splitn(3.max(take_col) + 1, '\t').collect::<Vec<_>>();
            if cols.len() < 3 {
                return Err(io::Error::new(io::ErrorKind::InvalidData, "Invalid bed: A minimum of 3 columns must be specified (chrom, start, end)."));
            }
