...
fn main() -> Result<(), BigBedReadAttachError> {
    ...
    let f = RemoteFile::new("http://users.wenglab.org/hueyj/temp/ENCSR711VWL/hg38/GRCh38-EDGEs.bigBed");
    let mut remote = BigBedRead::from(f)?;
    println!("remote info: {:?}", remote.info);

    let remote_intervals: Vec<_> = remote.get_interval("chr1", 12_244_400,12_248_000)?.collect::<Result<_,_>>()?;
