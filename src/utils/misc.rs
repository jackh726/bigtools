        ...
        move || -> Option<Result<BigWigAverageOverBedEntry, BBIReadError>> {
            ...
            match (|| -> Result<_, BBIReadError> {
                ...
                if take_col != 0 && cols.len() < take_col {
                    return Err(io::Error::new(io::ErrorKind::InvalidData, format!("Invalid bed: The column used for the name ({}) extends past the number of columns.", take_col)).into());
                }
