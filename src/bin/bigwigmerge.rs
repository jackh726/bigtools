...
fn main() -> Result<(), WriteGroupsError> {
    let matches = App::new("BigWigMerge")
        .arg(Arg::new("output")
                .about("the path of the merged output bigwig (if .bw or .bigWig) or bedGraph (if .bedGraph)")
                .index(1)
                .required(true)
            )
        .arg(Arg::new("bigwig")
                .short('b')
                .about("the path of an input bigwig to merge")
                .multiple(true)
                .takes_value(true)
            )
        .arg(Arg::new("list")
                .short('l')
                .about("a line-delimited list of bigwigs")
                .multiple(true)
                .takes_value(true)
            )
